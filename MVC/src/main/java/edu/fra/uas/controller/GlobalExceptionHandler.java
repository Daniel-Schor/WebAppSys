...
public class GlobalExceptionHandler {
	...
	public ModelAndView handleIllegalArgument(HttpServletRequest req, MethodArgumentTypeMismatchException exception) {
		log.debug("handleIllegalArgument() is called");

		ModelAndView mav = new ModelAndView();
		mav.addObject("exception", exception);
		mav.addObject("url", req.getRequestURL());
		mav.addObject("timestamp", new Date().toString());
		mav.addObject("status", 400); // Changed status to 400
